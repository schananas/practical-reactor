...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst(
                ((signal, messageFlux) ->
                        !signal.get().metaData.contains("spam") ? mailBoxPrimary() : mailBoxSecondary()));
    ...
    public Mono<String> chooseSource() {
        return Mono.defer(() ->
                sourceRef.get().equals("A") ? sourceA() : sourceRef.get().equals("B") ? sourceB() : Mono.empty()); //otherwise, return empty
    }
