...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        //todo: feel free to change code as you need
        Flux<Mono<Void>> rawTasks = taskExecutor();
        Flux<Void> tasks = rawTasks.flatMap(mono -> mono);
    ...
    public void streaming_service() {
        //todo: feel free to change code as you need
        Mono<Flux<Message>> messages = streamingService();
        Flux<Message> messageFlux = messages.flatMapMany(mes -> mes);
    ...
    public void i_am_rubber_you_are_glue() {
        //todo: feel free to change code as you need
        Flux<Integer> integerFlux = numberService1();
        Flux<Integer> integerFlux2 = numberService2();

        Flux<Integer> numbers = Flux.concat(integerFlux,integerFlux2);
    ...
    @Test
    public void i_am_rubber_you_are_glue_2() {
        //todo: feel free to change code as you need
        Flux<Integer> integerFlux = numberService1();
        Flux<Integer> integerFlux2 = numberService2();

        Flux<Integer> numbers = Flux.merge(integerFlux,integerFlux2);
    ...
    public void task_executor_again() {
        ...
        Flux<Void> tasks = monoFlux.concatMap(mono -> mono);
        //don't change below this line
        StepVerifier.create(tasks)
                .verifyComplete();
    ...
    public void need_for_speed() {
        ...
        Flux<String> stonks = Flux.firstWithSignal(stocksRest,stocksGrpc);
    ...
    public void plan_b() {
        ...
        Flux<String> stocksRest =  getStocksRest();
        Flux<String> stonks = Flux.firstWithValue(stocksLocalCache,stocksRest);
        //don't change below this line
        StepVerifier.create(stonks)
                .expectNextCount(6)
                .verifyComplete();
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> primary = mailBoxPrimary();
        Flux<Message> secondary = mailBoxSecondary();

        Flux<Message> myMail = primary
                .next()
                .flatMapMany(firstMessage -> {
                    if ("spam".equals(firstMessage.metaData)) {
                        return secondary;
                    } else {
                        return primary.concatWith(secondary);
                    }
                });
